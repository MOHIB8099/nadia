...
from voice_v16 import LocalWhisper, LunaVoiceOutput
from memory import LunaMemory
from actions_v16 import execute_action, get_due_reminders
from personality_v16 import PERSONALITY, recent_user_recall, style_instruction
...
class LunaApp:
    ...
    def __init__(self, root):
        ...
        self.speed_choice = tk.StringVar(value="Normal")
        self.style_choice = tk.StringVar(value="Confident")
        self.intensity_choice = tk.StringVar(value="Medium")
    ...
    def build_interface(self):
        ...
        crest = tk.Canvas(brand, width=66, height=62, bg="#02070D", highlightthickness=0)
        crest.pack(side="left", padx=(0, 10))
        crest.create_polygon(33, 3, 58, 17, 58, 46, 33, 60, 8, 46, 8, 17,
                             outline="#00D7F3", fill="#061A26", width=2)
        crest.create_polygon(33, 10, 52, 21, 52, 42, 33, 53, 14, 42, 14, 21,
                             outline="#155F79", fill="", width=1)
        crest.create_arc(17, 15, 49, 47, start=30, extent=112, style="arc",
                         outline="#76E9FF", width=2)
        crest.create_arc(17, 15, 49, 47, start=206, extent=107, style="arc",
                         outline="#76E9FF", width=2)
        crest.create_text(33, 31, text="L", font=("Segoe UI", 22, "bold"), fill="#E8FDFF")

        brand_text = tk.Frame(brand, bg="#02070D")
        brand_text.pack(side="left", fill="y")
        tk.Label(
            brand_text,
            text="LUNA // COMMAND CORE",
        ...
        tk.Label(
            voice_settings, text="VOICE // STYLE", font=("Consolas", 7, "bold"),
            bg="#04101A", fg="#4C7784"
        ).pack(anchor="w", pady=(8, 0))
        style_menu = tk.OptionMenu(
            voice_settings, self.style_choice,
            "Confident", "Sweet", "Playful", "Calm", "Focused", command=self.change_style
        )
        style_menu.config(bg="#082330", fg="#00E5FF", relief="flat", highlightthickness=0)
        style_menu.pack(fill="x", pady=(3, 0))

        tk.Label(
            voice_settings, text="STYLE // STRENGTH", font=("Consolas", 7, "bold"),
            bg="#04101A", fg="#4C7784"
        ).pack(anchor="w", pady=(8, 0))
        intensity_menu = tk.OptionMenu(
            voice_settings, self.intensity_choice,
            "Low", "Medium", "High", command=self.change_intensity
        )
        intensity_menu.config(bg="#082330", fg="#00E5FF", relief="flat", highlightthickness=0)
        intensity_menu.pack(fill="x", pady=(3, 0))
        ...
        self.voice_button.pack(side="left", fill="x", expand=True, ipady=7, padx=4)
    ...
    def change_style(self, value=None):
        selected = value or self.style_choice.get()
        self.voice.set_style(selected)
        self.add_message("SYSTEM", "Voice style: " + selected + ".")

    def change_intensity(self, value=None):
        selected = value or self.intensity_choice.get()
        self.voice.set_intensity(selected)
        self.add_message("SYSTEM", "Style strength: " + selected + ".")
    ...
    def set_state(self, state):
        ...
        if hasattr(self, "voice_button"):
            self.voice_button.config(
                text="■ STOP VOICE" if self.continuous_voice or state == "SPEAKING"
                else "🎤 START VOICE"
            )
    ...
    def animate_core(self):
        ...
        try:
            ...
            # Angular identity mark instead of an anonymous glowing dot.
            mark = max(26, core * 0.90)
            self.canvas.create_polygon(
                cx, cy-mark, cx+mark*0.86, cy-mark*0.48,
                cx+mark*0.86, cy+mark*0.48, cx, cy+mark,
                cx-mark*0.86, cy+mark*0.48,
                cx-mark*0.86, cy-mark*0.48,
                fill="#04212D", outline="#B8FCFF", width=2
            )
            self.canvas.create_text(
                cx, cy, text="L", font=("Segoe UI", 30, "bold"), fill="#E3FCFF"
            )
    ...
    def language_instruction(
        ...
        if language == "hi":
            ...
            return """
The user is speaking Hindi. Match their writing script and tone; Roman Hindi stays Roman Hindi unless requested otherwise.
"""

        elif language == "bn":

            return """
The user is speaking Bengali. Match their writing script and tone; Roman Bengali stays Roman Bengali unless requested otherwise.
"""

        else:

            return """
Match the language and writing script of the latest user message. Roman Hindi and Roman Bengali should stay in Roman script.
"""
    ...
    def toggle_voice_mode(self):
        ...
        if self.state == "SPEAKING" and not self.continuous_voice:
            self.voice.stop()
        ...
        try:
            ...
            while self.continuous_voice:
                ...
                if not text:
                    ...
                    reason = getattr(self.listener, "last_status", "no_voice")
                    if time.monotonic() - self.last_mic_hint_at > 45:
                        self.last_mic_hint_at = time.monotonic()
                        hint = {
                            "no_voice": "Mic ne awaaz nahi pakdi. Mic ke kareeb bolkar dekho.",
                            "too_quiet": "Awaaz bahut halki mili. Mic volume check karo.",
                            "not_recognized": "Awaaz mili, lekin shabd samajh nahi aaye. Dobara bolo.",
                            "mic_error": "Mic open nahi ho paya. Windows input device check karo.",
                            "recognition_error": "Voice recognition mein error aaya. Terminal check karo.",
                        }.get(reason, "Dobara bolkar dekho.")
                        self.root.after(0, lambda h=hint: self.add_message("SYSTEM", h))
                    continue
    ...
    def process_command(
        ...
        try:
            ...
            action_result = recent_user_recall(
                user_text,
                self.memory.get_history(limit=12)
            )
            if action_result is None:
                action_result = execute_action(user_text)
...
{style_instruction(self.style_choice.get(), self.intensity_choice.get())}
    ...
    def check_luna_reminders():
        try:
            for item in get_due_reminders():
                ...
                app.add_message("LUNA", alert)
                threading.Thread(
                    target=app.voice.speak,
                    args=(alert,),
                    daemon=True
                ).start()
        except Exception as error:
