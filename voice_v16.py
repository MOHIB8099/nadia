...
from personality_v16 import INTENSITIES, VOICE_STYLES, speech_plan
...
START_TIMEOUT = 12.0
# The default mic threshold is calibrated from the first quiet audio frames.
...
def calibrated_speech_threshold(ambient_samples):
    """Estimate a mic-specific speech threshold from quiet initial frames."""
    ambient = float(np.median(ambient_samples))
    return max(70.0, min(550.0, ambient * 2.3 + 35.0))
...
class LocalWhisper:
    ...
    def listen(self):
        ...
        def audio_callback(indata, frames, time_info, status):
            ...
            # Measure ambient noise briefly. A quiet microphone should not
            # need the same loudness threshold as a noisy one.
            if not speech_started and len(calibration) < CALIBRATION_FRAMES:
                calibration.append(rms)
                if len(calibration) == CALIBRATION_FRAMES:
                    speech_threshold = calibrated_speech_threshold(calibration)
                    print("Mic threshold:", round(speech_threshold))
            ...
            if rms >= speech_threshold and (
                len(calibration) >= CALIBRATION_FRAMES or rms >= 350.0
            ):
        ...
        try:
            ...
            segments, info = (
                self.model.transcribe(
                    ...
                    beam_size=1,
                    vad_filter=True,
                    vad_parameters=dict(min_speech_duration_ms=100, speech_pad_ms=250),
            ...
            self.last_status = "recognition_error"
            print(
                "Whisper Error:",
                error
            )
        ...
        if not text:
            ...
            self.last_status = "not_recognized"
            print(
...
class LunaVoiceOutput:
    ...
    def __init__(self, energy_callback=None):
        ...
        self.speech_speed = "Normal"
        self.speech_style = "Confident"
        self.style_intensity = "Medium"
    ...
    def set_speed(self, speed):
        if speed in {"Gentle", "Warm", "Normal", "Fast", "Very fast"}:
    ...
    def set_intensity(self, intensity):
        if intensity in INTENSITIES:
            self.style_intensity = intensity

    def stop(self):
        """Interrupt playback; speech still being generated is skipped afterwards."""
    ...
    def speak(
        ...
        with self._speak_lock:
            ...
            plan = speech_plan(text, self.speech_style, self.style_intensity)
            if not plan:
                return

            selected_voice = self.select_voice(text)
            print("Selected voice:", selected_voice)
            audio_files = [
                os.path.join(tempfile.gettempdir(),
                             "luna_v15_%s_%s_%s.mp3" % (os.getpid(), threading.get_ident(), index))
            ...
            try:
                ...
                for audio_file, (_, _, _, _, pause) in zip(audio_files, plan):
                    ...
                    if pause and self._stop_event.wait(pause):
                        break
            finally:
